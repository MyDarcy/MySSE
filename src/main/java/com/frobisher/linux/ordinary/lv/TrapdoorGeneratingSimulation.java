...
//		System.out.println("not hyper sequence:" + Arrays.toString(notHyperIncreasingSequence));
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < keywordNumberList.size(); i++) {
			System.out.println();
			int keywordNumber = keywordNumberList.get(i);
			int andNumbers = 2;
			int notNumbers = 3;
			int orNumbers = keywordNumber - andNumbers - notNumbers;
			List<List<Integer>> keywordsIndex = QuerySimulation.getThreeKindsOfKeywordsIndex(
					initialization, orNumbers, andNumbers, notNumbers);
