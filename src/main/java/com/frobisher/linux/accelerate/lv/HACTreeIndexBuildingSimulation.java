...
//		PriorityQueue<Double> tfIdfMaxHeap = new PriorityQueue<>(20, Comparator.reverseOrder());
		for (int i = 0; i < initialization.simulationDocumentNumber; i++) {
			...
			P[initialization.simulationDictSize - 1] = 1;
...
//		System.out.println("getPriorityQueue end.");
	...
	public List<double[]> getParentNodePruningVector(HACTreeNodePair pair) {
		double[] parent1 = new double[initialization.simulationDictSize];
		double[] parent2 = new double[initialization.simulationDictSize];
		for (int i = 0; i < initialization.simulationDictSize; i++) {
